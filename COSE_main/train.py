...
# ... other imports
from performance_metrics import PerformanceMetricsCallback
...
if __name__ == '__main__':
     ...
     for parser_ in (base_parser, parser):
          ...
          parser_.add_argument("--sde", type=str, choices=SDERegistry.get_all_names(), default="rectified_flow")
          ...
          parser_.add_argument("--save_ckpt_interval", type=int, default=10000000000, help="Save checkpoint interval.")
          
     temp_args, _ = base_parser.parse_known_args()
     print("Available backbone names:", BackboneRegistry.get_all_names())
     print("Available SDE names:", SDERegistry.get_all_names())
     ...
     print("Initializing ScoreModel...")
     model = ScoreModel(
          backbone=args.backbone, sde=args.sde, data_module_cls=data_module_cls,theta=0.5,  
          sigma_min=0.1,
          sigma_max=1.0, 
          **{
     ...
     callbacks = [ModelCheckpoint(dirpath=checkpoint_dir, save_last=True, 
          filename='{epoch}-last')]
     ...
     if args.num_eval_files:
          checkpoint_callback_pesq = ModelCheckpoint(dirpath=checkpoint_dir, 
               save_top_k=1, monitor="pesq", mode="max", filename='{epoch}-{pesq:.2f}')
          checkpoint_callback_si_sdr = ModelCheckpoint(dirpath=checkpoint_dir, 
     ...
     trainer = pl.Trainer(
          **vars(arg_groups['Trainer']),
          ...
          callbacks=callbacks + [PerformanceMetricsCallback()],
