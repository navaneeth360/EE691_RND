    ...
    def reverse(oself, score_model, probability_flow=False):
        ...
        class RSDE(oself.__class__):
            ...
            def discretize(self, x, y, t, stepsize):
                ...
                rev_f = f - G ** 2 * score_model(x, y, t) * (0.5 if self.probability_flow else 1.)
...
@SDERegistry.register("rectified_flow")
class RectifiedFlowSDE(SDE):
    def __init__(self, theta, sigma_min, sigma_max, eta_const=0.1, sigma_const=0.2, N=100, **kwargs):
        super().__init__(N)
    ...
    @property
    def T(self):
        """Return the end time of the SDE process."""
        return 1.0  # 根据实际需求设置终止时间

    @staticmethod
    def add_argparse_args(parent_parser):
        """Add SDE-specific arguments to argparse parser."""
        parser = parent_parser.add_argument_group("RectifiedFlowSDE")
        parser.add_argument("--sigma_min", type=float, default=0.1, help="Minimum diffusion coefficient.")
        parser.add_argument("--sigma_max", type=float, default=1.0, help="Maximum diffusion coefficient.")
        parser.add_argument("--theta", type=float, default=1.5, help="The constant stiffness of the Ornstein-Uhlenbeck process. 1.5 by default.")
        parser.add_argument("--sigma-min", type=float, default=0.05, help="The minimum sigma to use. 0.05 by default.")
        parser.add_argument("--sigma-max", type=float, default=0.5, help="The maximum sigma to use. 0.5 by default.")
        # 根据需求添加其他参数
    ...
    def copy(self):
        """Return a copy of the current SDE instance."""
        return RectifiedFlowSDE(sigma_min=self.sigma_min, sigma_max=self.sigma_max, theta=self.theta)

    def prior_sampling(self, shape):
        """Generate samples from the prior distribution."""
        return torch.randn(shape) * self.sigma(0)

    def prior_logp(self, z):
        """Compute the log probability of the prior distribution."""
        return -0.5 * z.pow(2).sum(dim=-1) - 0.5 * z.shape[-1] * torch.log(2 * torch.pi)

    def eta(self, t):
        """Calculate η_t as a constant."""
        return self.eta_const

    def sigma(self, t):
        """Calculate σ_t as a constant."""
        return self.sigma_const

    def alpha(self, t):
        """Calculate α_t = exp(∫_t^1 η_s ds), assuming η_t is constant."""
        eta_t = self.eta(t)
        return torch.exp(eta_t * (1 - t))

    def beta(self, t):
        """Calculate β_t^2 = ∫_t^1 exp(2∫_t^s η_r dr) * σ_s^2 ds, assuming η_t and σ_t are constant."""
        eta_t = self.eta(t)
        sigma_t = self.sigma(t)
        beta_squared = (sigma_t**2) * (1 - torch.exp(-2 * eta_t * (1 - t))) / (2 * eta_t)
        return torch.sqrt(beta_squared)

    def sde(self, x, t):
        """
        Calculate drift term for SDE using initial clean state X1, as per the article.

        Parameters:
        - x1: The initial clean signal, representing X1.
        - t: Current time step.
        """
        ...
        # Define V_t based on α_t, β_t, and initial clean state X1
        vt = alpha_t * x + beta_t * torch.randn_like(x)  # This is V_t

        # Compute the drift term based on the article's definition of Y_t
        drift = -eta_t * vt - (sigma_t ** 2 / (beta_t)) * torch.randn_like(x)
        diffusion = sigma_t
        return drift,diffusion
    def marginal_prob(self, x0, t):
        """
        Computes the marginal distribution parameters for p(x_t | x0) at time t.

        Parameters:
        - x0: Initial state (X_1), representing clean data.
        - t: Current time step.
        - xi_t: The sampled noise term for this time step.

        Returns:
        - mean: Mean of the marginal distribution at time t.
        - std: Standard deviation of the marginal distribution at time t.
        """
        alpha_t = self.alpha(t).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)  # 将 alpha_t 扩展为 [8, 1, 1, 1] 的形状
        beta_t = self.beta(t).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)    # 同样处理 beta_t
        # Calculate mean and std based on x0 and xi_t
        mean = alpha_t * x0
        std = beta_t
        return mean, std
    def _std(self, t):
    ...
    def reverse(oself, score_model, probability_flow=False):
        ...
        class RSDE(oself.__class__):
            ...
            def rsde_parts(self, x, y, t, *args):
                ...
                diffusion = sde_diffusion if self.probability_flow else sde_diffusion
                total_drift = sde_drift + score_drift
                return {
                    'total_drift': total_drift, 'diffusion': diffusion, 'sde_drift': sde_drift,
                    'sde_diffusion': sde_diffusion, 'score_drift': score_drift, 'score': score,
